...
$(document).ready(function(){ 
	...
	uploadUrl: "../uploadfiles.php?funct=uploadImagesFromGM"				// Server side upload url                        
    };
...
function multiUploader(config){    
	...
	_uploader = function(file,f){                        
            ...
            if(typeof file[f] != undefined && self._validate(file[f].type) > 0){                                                
                        ...
                        var galeriaId = $("#id_galeria").val();
                        data.append('galeriaId',galeriaId);
                        ...
                        $.ajax({
                            ...
                            success:function(rponse){
                                ...
                                //Recarga la pagina al terminar de subir los archivos
                                recargarPagina(totalImg, totalServer);
                            }                                
                        });                        
                } else{
	...
	_startUpload = function(){ 
        ...
        //Verificar qeu seleccione una galeria
        var id_galeria = $("#id_galeria").val();    
        if(id_galeria==""){
            alertify.error("Seleccionar una galer&iacute;a");
            return false;
        }
...
function recargarPagina(totalImg, totalServer){    
    // console.log(totalImg +" - "+ totalServer);
    if(totalImg==totalServer){
        setTimeout(function(){           
           location.reload();
        }, 500);
    }
