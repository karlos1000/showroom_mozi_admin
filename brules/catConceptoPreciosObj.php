...
include_once  $dirname.'/database/catConceptoPreciosDB.php';

class catConceptoPreciosObj {
    ...
    public function ObtTodosCatConceptoPrecios(){
        $array = array();
        $ds = new catConceptoPreciosDB();

        $result = $ds->ObtTodosCatConceptoPreciosDB();
        $array = $this->arrDatos($result, "concPrecioId");

        return $array;
    }


    //Grid concepto precios
    public function GetCatConceptoPreciosGrid(){
        $DataServices = new DataServices();
        $dbConn = $DataServices->getConnection();
        $ds = new MySQLiDataSource($dbConn);
        $uDB = new catConceptoPreciosDB();
        $ds = $uDB->catConceptoPreciosDataSet($ds);
        $grid = new KoolGrid("catConceptoPreciosGrid");

        $this->defineGridForm($grid, $ds);
        $this->defineColumnForm($grid, "concPrecioId", "ID", false, true);
        $this->defineColumnForm($grid, "concepto", "Nombre", true, false, 1);
